    ...
    def __init__(self, channels, kernel_size=3, dilation=(1, 3, 5)):
        ...
        self.convs1 = nn.ModuleList([
            weight_norm(Conv1d(channels, channels, kernel_size, 1, dilation=dilation[0],
                               padding=2*get_padding(kernel_size, dilation[0]))),
            weight_norm(Conv1d(channels, channels, kernel_size, 1, dilation=dilation[1],
                               padding=2*get_padding(kernel_size, dilation[1]))),
            weight_norm(Conv1d(channels, channels, kernel_size, 1, dilation=dilation[2],
                               padding=2*get_padding(kernel_size, dilation[2])))
        ])
        self.convs1.apply(init_weights)

        self.convs2 = nn.ModuleList([
            weight_norm(Conv1d(channels, channels, kernel_size, 1, dilation=1,
                               padding=2*get_padding(kernel_size, 1))),
            weight_norm(Conv1d(channels, channels, kernel_size, 1, dilation=1,
                               padding=2*get_padding(kernel_size, 1))),
            weight_norm(Conv1d(channels, channels, kernel_size, 1, dilation=1,
                               padding=2*get_padding(kernel_size, 1)))
...
class NSPP_Model(torch.nn.Module):
    def __init__(self, h):
        ...
        self.input_conv = weight_norm(Conv1d(h.n_fft//2+1, h.ResNet_channel, h.ResNet_input_conv_kernel_size, 1, 
                                             padding=2*get_padding(h.ResNet_input_conv_kernel_size, 1)))
        ...
        self.PEA_conv_R = weight_norm(Conv1d(h.ResNet_channel, h.n_fft//2+1, h.PEA_R_conv_kernel_size, 1, 
                                             padding=2*get_padding(h.PEA_R_conv_kernel_size, 1)))
        self.PEA_conv_I = weight_norm(Conv1d(h.ResNet_channel, h.n_fft//2+1, h.PEA_I_conv_kernel_size, 1, 
                                             padding=2*get_padding(h.PEA_I_conv_kernel_size, 1)))
    ...
    def forward(self, log_amp):
        ...
        pseudo_real_part = pseudo_real_part[:, :, 0 : split]
        output_list.append(pseudo_real_part)
        pseudo_imaginary_part = self.PEA_conv_I(output)
        pseudo_imaginary_part = pseudo_imaginary_part[:, :, 0 : split]
        output_list.append(pseudo_imaginary_part)

        phase = torch.atan2(pseudo_imaginary_part, pseudo_real_part)

        return phase, output_list
...
def MSE_loss(feature_r, feature_g):

    MSELoss = torch.nn.MSELoss()

    MSE = MSELoss(feature_r, feature_g)
