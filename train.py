...
from models import NSPP_Model, losses, MSE_loss
from models_teacher import NSPP_Model as NSPP_teacher_model
...
def train(h):
    ...
    NSPP_teacher = NSPP_teacher_model(h).to(device)
    state_dict_teacher = load_checkpoint(h.teacher_checkpoint_file_load, device)
    NSPP_teacher.load_state_dict(state_dict_teacher['model'])
    ...
    for epoch in range(max(0, last_epoch), h.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            _, teacher_KD = NSPP_teacher(log_amplitude)

            phase_g, student_KD = NSPP(log_amplitude)
            ...
            for j in range(len(teacher_KD)):

                loss_KD += MSE_loss(teacher_KD[j], student_KD[j])

            loss_all = L_IP + L_GD + L_IAF + 0.05* loss_KD

            loss_all.backward()
            optim.step()

            if steps % h.stdout_interval == 0:
                print('Steps : {:d}, Total Loss: {:4.3f}, Instantaneous Phase Loss : {:4.3f}, Group Delay Loss : {:4.3f}, Instantaneous Angular Frequency Loss : {:4.3f}, Knowledge Distillation Loss : {:4.3f},  s/b : {:4.3f}'.
                      format(steps, loss_all, L_IP, L_GD, L_IAF, loss_KD, time.time() - start_b))
            ...
            if steps % h.validation_interval == 0 and steps != 0:
                ...
                with torch.no_grad():
                    for j, batch in enumerate(validation_loader):
                        ...
                        _, teacher_KD = NSPP_teacher(log_amplitude.to(device))
                        phase_g, student_KD = NSPP(log_amplitude.to(device))
                        phase = torch.autograd.Variable(phase.to(device, non_blocking=True))

                        val_L_IP, val_L_GD, val_L_IAF = losses(phase, phase_g, h.n_fft, phase.size()[-1])
                        val_L_KD = 0
                        for k in range(len(teacher_KD)):
                            val_L_KD += MSE_loss(teacher_KD[k], student_KD[k])
                        val_L_IP_total += val_L_IP.item()
                        val_L_GD_total += val_L_GD.item()
                        val_L_IAF_total += val_L_IAF.item()
                        val_L_KD_total += val_L_KD.item()


                    val_L_IP = val_L_IP_total / (j+1)
                    val_L_GD = val_L_GD_total / (j+1)
                    val_L_IAF = val_L_IAF_total / (j+1)
                    val_L_KD = val_L_KD_total / (j+1)

                    sw.add_scalar("Validation/Total Loss", val_L_IP + val_L_GD + val_L_IAF + 0.05* val_L_KD, steps)
                    ...
                    sw.add_scalar("Validation/Knowledge Distillation Loss", val_L_KD, steps)
